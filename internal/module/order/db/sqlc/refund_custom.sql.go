...
func (q *Queries) ListSellerRefunds(ctx context.Context, arg ListSellerRefundsParams) ([]ListSellerRefundsRow, error) {
	rows, err := q.db.Query(ctx, listSellerRefunds, arg.SellerID, arg.Offset, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListSellerRefundsRow{}
	for rows.Next() {
		var i ListSellerRefundsRow
		if err := rows.Scan(
			&i.OrderRefund.ID,
