...
// InsertRefund opens a case. One live refund per order — a refund covers the whole order,
// so a second could not be about anything — held by the partial unique index.
func (r *Repo) InsertRefund(ctx context.Context, ref *domain.Refund) error {
	const q = `INSERT INTO refund (buyer_id, order_id, reason, attachments, status, deadline_at)
	           VALUES (@buyer_id, @order_id, @reason, @attachments, @status, @deadline_at)
	...
	if err := r.pool.QueryRow(ctx, q, args).Scan(&ref.ID, &ref.CreatedAt); err != nil {
...
func (r *Repo) FindOpenRefundByOrder(ctx context.Context, orderID int64) (domain.Refund, error) {
	const q = `SELECT ` + refundColumns + ` FROM refund
	           WHERE order_id = @order_id AND status NOT IN ('accepted', 'rejected')`
...
func (r *Repo) ListRefunds(ctx context.Context, f port.RefundFilter) ([]domain.Refund, error) {
	...
	const q = `SELECT ` + refundColumns + ` FROM refund r
	           WHERE (@buyer_id = 0 OR r.buyer_id = @buyer_id)
	             ...
	             AND (@before::timestamptz IS NULL OR r.created_at < @before::timestamptz)
	           ORDER BY r.created_at DESC, r.id DESC
	           LIMIT @limit`
	before, limit := cursorBound(f.Cursor)
	args := pgx.NamedArgs{
		"buyer_id": f.BuyerID, "seller_id": f.SellerID,
		"statuses": nullStrings(f.Statuses), "before": before, "limit": limit,
	}
...
func (r *Repo) SaveRefund(ctx context.Context, ref domain.Refund) error {
	const q = `UPDATE refund
	           SET status = @status, deadline_at = @deadline_at, attachments = @attachments,
	               seller_decided_at = @seller_decided_at, rejection_reason = @rejection_reason,
	               return_transport_id = @return_transport_id, returned_at = @returned_at,
...
func (r *Repo) ListOpenDisputes(ctx context.Context, f port.CursorFilter) ([]domain.Dispute, error) {
	...
	args := pgx.NamedArgs{"before": before, "limit": limit}
	rows, err := r.pool.Query(ctx, q, args)
	if err != nil {
...
// SaveDispute records the verdict. `WHERE status = 'open'` is the transition: a round is
// ruled once, because a later round is argued against what the earlier one decided.
func (r *Repo) SaveDispute(ctx context.Context, d domain.Dispute) error {
	const q = `UPDATE refund_dispute
	           SET status = @status, resolved_by_id = @resolved_by, resolved_at = @resolved_at,
