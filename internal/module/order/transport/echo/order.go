...
func NewHandler(
	...
	for _, opt := range opts {
		...
		if key := client.WireWebhooks(e, h.biz.Call().OnPaymentResult, registered); key != "" {
	...
	onTransportResult := func(ctx context.Context, result transport.WebhookResult) error {
		...
		return biz.Call().OnTransportResult(ctx, orderbiz.OnTransportResultParams{
...
func (h *Handler) CancelBuyerPending(c echo.Context) error {
	...
	if err := h.biz.Call().CancelBuyerPending(c.Request().Context(), orderbiz.CancelBuyerPendingParams{
		AccountID: claims.Account.ID,
		ItemID:    itemID,
	}); err != nil {
		return response.FromError(c.Response().Writer, http.StatusInternalServerError, err)
