...
WHERE (
    ...
    (
      ("date_deleted" IS NULL) AND (
        ((catalog.f_unaccent("slug") ILIKE '%' || catalog.f_unaccent($12) || '%') OR $12 IS NULL) AND
        ((catalog.f_unaccent("name") ILIKE '%' || catalog.f_unaccent($13) || '%') OR $13 IS NULL) AND
        ((catalog.f_unaccent("description") ILIKE '%' || catalog.f_unaccent($14) || '%') OR $14 IS NULL)
      )
    )
)
ORDER BY "id"
