...
func userTopic() bus.Topic[userEvent] {
	return bus.NewTopic[userEvent]("user.test")
}

func TestPublishSubscribeRoundtrip(t *testing.T) {
	t.Parallel()
	mem := bus.NewMemory(nil)
	c := bus.Client(mem)
	topic := userTopic()
	...
	if err := bus.Publish(context.Background(), c, topic, userEvent{ID: 1, Name: "a"}); err != nil {
		t.Fatal(err)
	}
	mem.Wait()
...
func TestEachGroupDeliveredOnce(t *testing.T) {
	...
	bus.Subscribe(c, topic, "g1", func(_ context.Context, _ userEvent) error {
		g1.Add(1)
		return nil
	})
	bus.Subscribe(c, topic, "g2", func(_ context.Context, _ userEvent) error {
		g2.Add(1)
		return nil
	})
...
func TestCompetingConsumersRoundRobin(t *testing.T) {
	...
	for i := range 4 {
		if err := bus.Publish(context.Background(), c, topic, userEvent{ID: i, Name: "a"}); err != nil {
			t.Fatal(err)
		}
	}
	mem.Wait()
...
func TestPublishNoSubscribers(t *testing.T) {
	...
	if err := bus.Publish(context.Background(), c, userTopic(), userEvent{ID: 1, Name: "a"}); err != nil {
		t.Fatal(err)
	}
	mem.Wait()
}
...
func TestBatchFlushBySize(t *testing.T) {
	...
	bus.SubscribeBatch(c, topic, "g1", func(_ context.Context, payloads []userEvent) error {
		...
		return nil
	}, bus.WithBatchSize(3), bus.WithLinger(time.Minute))
...
func TestBatchFlushByLinger(t *testing.T) {
	...
	bus.SubscribeBatch(c, topic, "g1", func(_ context.Context, payloads []userEvent) error {
		...
		batches = append(batches, payloads)
		return nil
	}, bus.WithBatchSize(100), bus.WithLinger(50*time.Millisecond))
...
func TestPublishAfterClose(t *testing.T) {
	...
	bus.Subscribe(c, topic, "g1", func(_ context.Context, _ userEvent) error { return nil })
	mem.Close()

	err := bus.Publish(context.Background(), c, topic, userEvent{ID: 1, Name: "a"})
...
func TestCloseDrainsPending(t *testing.T) {
	...
	bus.SubscribeBatch(c, topic, "g1", func(_ context.Context, payloads []userEvent) error {
		...
		return nil
	}, bus.WithBatchSize(100), bus.WithLinger(time.Minute))
